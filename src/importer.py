...
class TempCategory:
	...
	def clean_up(self):
		...
		for i in self.data:
			...
			if clean[0] == "visgroupid" or clean[0] == "groupid": 	#We can't store these as simple key/val pairs in a dict, because they're the only VMF attributes that can occur on multiple lines for a single item (e.g. if item is in multiple visgroups).
				if clean[0] in self.dic:							#If we store as a single key/val pair, we can only keep track of one visgroupid and one groupid at a time
					self.dic[clean[0]].append(num(clean[1]))		#So we have to store as a key/list pair, where key is "visgroupid" or "groupid" and list is the list of the item's visgroup or group ids
				else:												#I don't know why Valve didn't just make it so you could store multiple visgroupids and groupids under a single visgroupid/groupid attribute, like with info_overlay brush faces
					self.dic[clean[0]] = [num(clean[1])]
			else:
